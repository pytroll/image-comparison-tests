...
`ansible-playbook -i inventory.ini --ask-become-pass playbooks/deploy_image_comparison.yml -u your_user --ask-become-pass --ask-pass`.
- Before the server can start, a file `/home/bildabgleich/pytroll-image-comparison-tests/serverLogic/secret.py` must exist, in which the Webhook-Secret and Github-Token are stored in the following format:
```
WEBHOOK_SECRET = "xxx"
...
Ansible is an open-source automation tool used for configuration management, application deployment, and task automation. In this project, Ansible is used to automate the setup of the bildabgleich server.
...
- **roles/add_user**: Creates the `bildabgleich` user on the target system with limited shell access (`/bin/false`) for security. This ensures the service runs under a controlled user account for proper access management.
- **roles/configure_nginx**: Copies the Nginx configuration for the `bildabgleich` service from the template location, enables the configuration by creating a symbolic link, tests the Nginx configuration, and reloads Nginx if the configuration is valid.
- **roles/deploy_data**: Copies the `serverLogic` and `data` directories to the target system, ensuring they are owned by the `bildabgleich` user. It also converts Windows line endings in the `start_server.sh` script to Unix format, and sets appropriate permissions for executing the script.
